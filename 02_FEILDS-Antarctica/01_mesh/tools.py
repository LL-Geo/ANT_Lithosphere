...
class RasterData(object):
    ...
    def __init__(self, FN, X,Y, isCellCentered=False, **data):
        ...
        if self.isCellCentered:
           self.spacingX=(self.X.max()-self.X.min())/(self.dimensions[1]-1)
           self.spacingY=(self.Y.max()-self.Y.min())/(self.dimensions[0]-1)
           ...
           self.spacingX=(self.rangeX[1]-self.rangeX[0])/(self.dimensions[1]-1)
           self.spacingY=(self.rangeY[1]-self.rangeY[0])/(self.dimensions[0]-1)
    ...
    def interpolateToEscript(self, xyh, target, topography, moho, mantle):
        ...
        for p in range( target.getNumberOfDataPoints() ):
            ...
            if h > topography.getTopography(x,y):
                ...
                dd=np.interp(h, mantle.getDepth(x,y), mantle.getDensity(x,y), left=moho.getDensityCrust(x,y), right=moho.getDensityMantel(x,y), period=None)
                ...
                dd=np.interp(h, hs, ds, left=rho_c_top, right=rho_c_bottom, period=None)
